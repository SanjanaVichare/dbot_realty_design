...
// ─── Tag styles (mapped to palette) ──────────────────────────────────────────
const tagStyles: Record<string, { bg: string; color: string }> = {
  dept: { bg: C.limeTint, color: C.forest },      // Forest on Lime Tint
  loc: { bg: C.sand, color: "#7A5C00" },      // Warm amber-brown on Sand
  type: { bg: C.skyTint, color: C.royalBlue },    // Royal Blue on Sky Tint
  exp: { bg: C.iceBlue, color: C.charcoal },     // Charcoal Navy on Ice Blue
};
    ...
    <>
      ...
      <main>
        ...
        <section style={{
          ...
          gap: 40, flexWrap: "wrap", minHeight: 520,
        }}>

          {/* LEFT */}
          <div style={{ maxWidth: 520 }}>

            <h1 style={{ fontSize: "2.8rem", fontWeight: 800, color: C.midnight, lineHeight: 1.15, marginBottom: 16 }}>
              Build a career<br />you're proud of<br />at{" "}
              <span style={{ color: C.royalBlue }}>DBOT</span>
            </h1>

            <p style={{ color: C.charcoal, opacity: 0.65, fontSize: "1.05rem", lineHeight: 1.6, marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", gap: 28, marginBottom: 32, flexWrap: "wrap" }}>
              {[
                ...
                <div key={s.label} style={{
                  ...
                  borderLeft: i > 0 ? `1px solid ${C.skyTint}` : "none",
                }}>
                  <span style={{ fontSize: "1.4rem", fontWeight: 800, color: C.midnight }}>{s.num}</span>
                  <span style={{ fontSize: "0.75rem", color: C.charcoal, opacity: 0.5, marginTop: 2 }}>{s.label}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              <div style={{ position: "relative", display: "inline-block" }}>
                <button
                  ...
                  style={{
                    background: C.royalBlue, color: "#fff",
                ...
                {dropdownOpen && (
                  <div style={{
                    ...
                    boxShadow: "0 15px 40px rgba(0,18,64,0.10)",
                    overflow: "hidden", width: 220, zIndex: 10,
                    border: `1px solid ${C.skyTint}`,
                  }}>
                    {sections.map((s) => (
                      <div
                        key={s.id}
                        ...
                        style={{
                          ...
                          borderBottom: `1px solid ${C.skyTint}`,
                          fontSize: "0.9rem", color: C.midnight,
                        }}
                      >
                        {s.label}
                      </div>
              ...
              <a
                ...
                style={{
                  background: "#fff", color: C.royalBlue,
                  padding: "12px 20px", borderRadius: 12,
                  border: `1.5px solid ${C.skyTint}`,
          ...
          <div style={{ position: "relative", width: 320, height: 420, flexShrink: 0 }}>
            ...
            {/* Card 3 — back */}
            <div style={{
              width: 270, height: 360, borderRadius: 20,
            ...
            {/* Card 1 — front */}
            <div style={{
              width: 270, height: 360, background: C.royalBlue, borderRadius: 20,
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  ...
                  <div key={f.label} style={{ background: "rgba(255,255,255,0.12)", borderRadius: 10, padding: "11px 14px" }}>
                    <div style={{ fontSize: "0.8rem", color: "#fff", fontWeight: 700 }}>{f.label}</div>
                    <div style={{ fontSize: "0.7rem", color: "rgba(255,255,255,0.5)", marginTop: 2 }}>{f.sub}</div>
                  </div>
                ))}
              </div>
        ...
        <section id="life" style={{ padding: "90px 5%", background: "#fff" }}>
          <h2 style={{ fontSize: "1.75rem", fontWeight: 800, color: C.midnight }}>Life at DBOT</h2>
          <p style={{ color: C.charcoal, opacity: 0.55, fontSize: "0.95rem", marginBottom: 32 }}>What makes working here different</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px,1fr))", gap: 20 }}>
            {lifeCards.map((c) => (
              <div key={c.title} style={{ background: "#fff", borderRadius: 16, padding: 28, border: `1px solid ${C.skyTint}` }}>
                <div style={{
                ...
                <h3 style={{ fontSize: "1rem", fontWeight: 700, color: C.midnight, marginBottom: 6 }}>{c.title}</h3>
                <p style={{ fontSize: "0.875rem", color: C.charcoal, opacity: 0.7, lineHeight: 1.6 }}>{c.desc}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        {/* ── GALLERY ── */}
        <section id="gallery" style={{ padding: "90px 5%", background: C.iceBlue }}>
          <h2 style={{ textAlign: "center", fontSize: "2rem", fontWeight: 800, color: C.midnight, marginBottom: 6 }}>
            Life inside DBOT Realty
          </h2>
          <p style={{ textAlign: "center", color: C.charcoal, opacity: 0.55, fontSize: "0.95rem", marginBottom: 40 }}>
            A glimpse at the people, spaces, and moments that make us who we are
          </p>

          {/* Mosaic — uses palette tints */}
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10, maxWidth: 900, margin: "0 auto" }}>
            <div style={{ gridColumn: 1, gridRow: 1, height: 120, borderRadius: 14, background: C.skyTint }} />
            <div style={{ gridColumn: 1, gridRow: 2, height: 140, borderRadius: 14, background: C.limeTint }} />
            <div style={{ gridColumn: 1, gridRow: 3, height: 100, borderRadius: 14, background: C.sand }} />
            <div style={{ gridColumn: 2, gridRow: "1/3", height: 270, borderRadius: 14, background: "#A8C8E8" }} />
            <div style={{ gridColumn: 2, gridRow: 3, height: 100, borderRadius: 14, background: C.skyTint }} />
            <div style={{ gridColumn: 3, gridRow: "1/4", height: 370, borderRadius: 14, background: C.royalBlue }} />
            <div style={{ gridColumn: 4, gridRow: 1, height: 150, borderRadius: 14, background: C.limeTint }} />
            <div style={{ gridColumn: 4, gridRow: 2, height: 110, borderRadius: 14, background: C.sand }} />
            <div style={{ gridColumn: 4, gridRow: 3, height: 110, borderRadius: 14, background: C.skyTint }} />
            <div style={{ gridColumn: 5, gridRow: 1, height: 130, borderRadius: 14, background: C.iceBlue }} />
            <div style={{ gridColumn: 5, gridRow: "2/4", height: 240, borderRadius: 14, background: "#7BAE5A" }} />
          </div>

          {/* TESTIMONIALS */}
          <div style={{ maxWidth: 900, margin: "60px auto 0" }}>
            <p style={{ textAlign: "center", fontSize: "1.3rem", fontWeight: 800, color: C.midnight, marginBottom: 6 }}>
              Trusted by our people
            </p>
            <p style={{ textAlign: "center", fontSize: "0.9rem", color: C.charcoal, opacity: 0.5, marginBottom: 40 }}>
                ...
                <div key={t.name} style={{
                  ...
                  <div>
                    <div style={{ display: "flex", gap: 3, marginBottom: 14 }}>
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} width="14" height="14" viewBox="0 0 24 24" fill={C.forest} xmlns="http://www.w3.org/2000/svg">
                          <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                        </svg>
                      ))}
                    </div>
                    <p style={{ fontSize: "0.88rem", color: C.charcoal, lineHeight: 1.7, fontStyle: "italic" }}>
                      "{t.quote}"
                    </p>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, paddingTop: 16, borderTop: `1px solid ${C.iceBlue}` }}>
                    <div style={{
                    ...
                    <div>
                      <div style={{ fontSize: "0.88rem", fontWeight: 700, color: C.midnight }}>{t.name}</div>
                      <div style={{ fontSize: "0.75rem", color: C.charcoal, opacity: 0.5, marginTop: 2 }}>{t.role}</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <section id="values" style={{ padding: "90px 5%", background: "#fff" }}>
          <div style={{ maxWidth: 900, margin: "0 auto" }}>
            <h2 style={{ fontSize: "1.75rem", fontWeight: 800, color: C.midnight, marginBottom: 8 }}>The Values</h2>
            <p style={{ color: C.charcoal, opacity: 0.55, fontSize: "0.95rem", marginBottom: 40 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px,1fr))", gap: 16 }}>
              {values.map((v) => (
                <div key={v.label} style={{
                  ...
                  borderTop: `3px solid ${C.forest}`,   // Forest accent instead of blue
                }}>
                  <span style={{ fontSize: "0.7rem", fontWeight: 800, color: C.forest, letterSpacing: "0.1em", textTransform: "uppercase" }}>
                    {v.num}
                  </span>
                  <div style={{ fontSize: "1.1rem", fontWeight: 800, color: C.midnight }}>{v.label}</div>
                  <div style={{ fontSize: "0.85rem", color: C.charcoal, opacity: 0.65, lineHeight: 1.6 }}>{v.desc}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <section id="openings" style={{ padding: "90px 5%", background: "#fff" }}>
          <h2 style={{ fontSize: "1.75rem", fontWeight: 800, color: C.midnight }}>Current openings</h2>
          <p style={{ color: C.charcoal, opacity: 0.55, fontSize: "0.95rem", marginBottom: 8 }}>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      background: isOpen ? C.iceBlue : isHovered ? "#FAFCFF" : "#fff",
                      transition: "background 0.2s",
                    }}>
                      <div>
                        <div style={{ fontSize: "0.95rem", fontWeight: 700, color: C.midnight, marginBottom: 6 }}>
                          {job.title}
                        </div>
                      ...
                      <div style={{
                        ...
                        border: `1px solid ${isOpen || isHovered ? C.skyTint : C.iceBlue}`,
                        display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0,
                        background: isOpen ? C.royalBlue : isHovered ? C.skyTint : "#fff",
                        color: isOpen ? "#fff" : C.royalBlue,
                        fontSize: 12,
                    ...
                    {/* Expanded body */}
                    {isOpen && (
                      <div style={{ borderTop: `1px solid ${C.iceBlue}`, padding: "0 22px 22px" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginTop: 18 }}>
                          {[
                            ...
                            <div key={heading}>
                              <p style={{ fontSize: "0.75rem", fontWeight: 700, letterSpacing: "0.06em", color: C.charcoal, opacity: 0.45, textTransform: "uppercase", marginBottom: 10 }}>
                                {heading}
                              </p>
                              <ul style={{ listStyle: "none", padding: 0 }}>
                                {items.map((r) => (
                                  <li key={r} style={{ fontSize: "0.85rem", color: C.charcoal, lineHeight: 1.6, padding: "3px 0 3px 14px", position: "relative" }}>
                                    <span style={{ position: "absolute", left: 2, color: C.forest, fontSize: "1.2rem", lineHeight: 1.3 }}>·</span>
                                    {r}
                                  </li>
                                ))}
                              </ul>
                        ...
                        <div style={{ marginTop: 18, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 10 }}>
                          <span style={{ fontSize: "0.85rem", color: C.charcoal, opacity: 0.7 }}>
                            CTC: <strong style={{ color: C.midnight, opacity: 1 }}>{job.ctc}</strong>
                          </span>
                          <a
                            href="https://www.linkedin.com/company/dbot-realty/posts/?feedView=all"
                            target="_blank"
                            onClick={(e) => e.stopPropagation()}
                            style={{ background: C.royalBlue, color: "#fff", padding: "10px 22px", borderRadius: 10, fontSize: "0.85rem", fontWeight: 600, textDecoration: "none" }}
                          >
                            Apply on LinkedIn →
                          </a>
                        </div>
          ...
          <div style={{ textAlign: "center", marginTop: 40 }}>
            <a
              href="https://www.linkedin.com/company/dbot-realty/posts/?feedView=all"
              target="_blank"
              style={{ display: "inline-block", border: `1.5px solid ${C.royalBlue}`, color: C.royalBlue, padding: "12px 28px", borderRadius: 12, fontWeight: 600, textDecoration: "none", fontSize: "0.95rem" }}
            >
